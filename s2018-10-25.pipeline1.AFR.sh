...
python /netapp/home/dmctong/imp_rvat/50k_scripts/s2018-10-25.pipeline1.AFR.py $POP_SIZE $JOB_ID ${JOB_ID}.AFR-${POP_SIZE}.vcf.gz $TEMP_DIR


# rsync back to here:
mkdir /wynton/scratch/dmctong/imp_rvat/data/input-gzip/AFR/ # just in case
if [[ $EXIT_STATUS -eq "0" ]]; then
    rsync -rauz ${TEMP_DIR}/* /wynton/scratch/dmctong/imp_rvat/data/input-gzip/AFR/
    EXIT_STATUS=$(( ${EXIT_STATUS} || $? ))
    if [[ $EXIT_STATUS -ne "0" ]]; then
